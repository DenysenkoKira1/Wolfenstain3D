...
namespace Wolfenstain3D
{
    ...
    internal class Door
    {
        ...
        public event Action? OpeningStarted; // Подія для звуку відкривання дверей
        public event Action? ClosingStarted; // Подія для звуку закривання дверей
        ...
        public void StartOpening()
        {
            if (_state == DoorState.Closed || _state == DoorState.Closing)
            {
                ...
                OpeningStarted?.Invoke(); // Повідомляємо гру, що треба програти звук відкривання
            }
        }
        ...
        private void TryStartClosing(bool playerBlocksDoor)
        {
            ...
            {
                _state = DoorState.Closing; // Закриваємо тільки якщо минула пауза і гравець не стоїть у клітинці дверей
                ClosingStarted?.Invoke(); // Повідомляємо гру, що треба програти звук закривання
            }
        }
